...
function normalizeChunk(buffer) {
      ...
      if (block) {
        ...
        switch (event.event) {
          ...
          case "done":
            ...
            break;
          case "error":
            options.onError?.(typeof payload.message === "string" ? payload.message : "Unknown streaming error.");
            break;
          default:
            break;
        }
...
function appendChunk(messages, id, chunk) {
...
function WebApp() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const message = streamError instanceof Error ? streamError.message : "The chat request failed.";
      setError(message);
