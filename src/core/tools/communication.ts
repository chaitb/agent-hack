...
export function createCommunicationTools(db: DB, channels: CommChannels) {
	return {
		send_telegram: createToolHandler({
			namespace: "comm",
			name: "send_telegram",
			description: "Send a Telegram message to the user. Use this to proactively reach out.",
			db,
		...
		send_email: createToolHandler({
			namespace: "comm",
			name: "send_email",
			description: "Send an email to the user. Use for longer-form communication.",
			db,
			inputSchema: z.object({
			...
			execute: async ({ subject, body }) => {
				await db.saveMessage("assistant", `[Email: ${subject}]\n${body}`, "email");
