	...
	return {
		remember: createToolHandler({
			...
			inputSchema: z.object({
				key: z
					.string()
					.describe("Semantic key, e.g. 'user.timezone', 'user.name', 'preference.communication'"),
		...
		recall: createToolHandler({
			...
			inputSchema: z.object({
				key: z
					.string()
					.describe("Exact key for mode='exact', or a natural-language query for mode='search'"),
				mode: z
					.enum(["exact", "search"])
					.default("exact")
					.describe("'exact' for key lookup, 'search' for hybrid semantic + keyword search"),
				category: z
					.enum(["preference", "fact", "skill", "context"])
					.optional()
