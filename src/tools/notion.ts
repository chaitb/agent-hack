...
export function createNotionTools(db: DB) {
  ...
  return {
    notion_search: createToolHandler({
      ...
      inputSchema: z.object({
        query: z
          .string()
          .optional()
          .describe("Search query. Omit to list recent items."),
    ...
    notion_create_page: createToolHandler({
      ...
      name: "create_page",
      description:
        "Create a new page in Notion. Can be a child of a page or database entry. Supports markdown.",
      db,
      inputSchema: z.object({
        ...
        properties: z
          ...
          .optional()
          .describe("Additional properties for database entries"),
      }),
      execute: async ({
        parent_type,
        parent_id,
    ...
    notion_get_block_children: createToolHandler({
      ...
      description:
        "Get the content blocks of a Notion page. Use to read page content.",
      db,
      inputSchema: z.object({
        block_id: z.string(),
...
function extractTitle(obj: Record<string, unknown>): string {
  try {
    ...
    if (obj.title && Array.isArray(obj.title)) {
      return (
        (obj.title as Array<{ plain_text?: string }>)
          .map((t) => t.plain_text)
          .join("") || "(untitled)"
      );
    }
