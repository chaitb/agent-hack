...
export function createFileTools(db: DB) {
  return {
    ...
    read_instructions: createToolHandler({
      ...
      description:
        "Read a specific instruction/skill file from the instructions directory.",
      db,
      inputSchema: z.object({
        filename: z
          .string()
          .describe("e.g. 'system-prompt.md' or 'skills/email-summary.md'"),
      }),
    ...
    update_instructions: createToolHandler({
      ...
      description:
        "Write or update an instruction/skill file. This is how the agent evolves its own behavior.",
      db,
      inputSchema: z.object({
        ...
        description: z
          .string()
          .optional()
          .describe("Brief description of this instruction file"),
      }),
      execute: async ({ filename, content, description }) => {
        ...
        await db.upsertInstruction(
          filename,
          description ?? `Instruction file: ${filename}`,
        );
    ...
    list_skills: createToolHandler({
      ...
      name: "list_skills",
      description:
        "List all instruction and skill files available to the agent.",
      db,
      inputSchema: z.object({}),
      execute: async () => {
        const files: string[] = [];
