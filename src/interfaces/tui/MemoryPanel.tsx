...
export const MemoryPanel = ({ db, onBack }: { db?: DB; onBack: () => void }) => {
	...
	return (
		...
		>
			<Box flexDirection="column" width={"30%"}>
				<Text color="cyan">
					[{memory.id.slice(0, 8)}]{memory.score ? ` Score: (${memory.score.toFixed(2)})` : ""}
				</Text>
			...
			<Box flexGrow={1} width={"70%"} flexDirection="column" paddingX={1}>
				...
				<Spacer />
				<Text dimColor>Last Updated: {new Date(memory.updated_at).toLocaleString()}</Text>
			</Box>
		</Box>
	);
}
