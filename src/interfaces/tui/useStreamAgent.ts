...
import type { ChatService, ChatMessage } from "../../core/chat";
...
export function useStreamAgent(chatService: ChatService | undefined) {
  ...
  const loadHistory = useCallback(async (service = chatService) => {
    if (!service) return;
    const loaded = await service.getRecentMessages(20);
    if (loaded.length > 0) {
      setMessages(loaded);
    }
  }, [chatService]);
  ...
  const send = useCallback(
    async (input: string) => {
      if (!chatService || isStreaming) return;
      ...
      try {
        for await (const chunk of chatService.streamReply(input, "cli")) {
          if (!started) {
