import type { DB, Task } from "./database";
    ...
    try {
      ...
      if (task.recurrence) {
        ...
        if (nextRun) {
          ...
          logger.push(
            "heartbeat",
            `Rescheduled: ${task.title} → ${nextRun.toISOString()}`,
          );
        }
      }
      ...
      logger.push(
        "heartbeat",
        `Failed: ${task.title} — ${(err as Error).message}`,
      );
    }
  }
}
...
function calculateNextRun(recurrence: string): Date | null {
  ...
  if (everyMinMatch) {
    return new Date(
      now.getTime() + Number.parseInt(everyMinMatch[1]!) * 60_000,
    );
  }
  ...
  if (everyHourMatch) {
    return new Date(
      now.getTime() + Number.parseInt(everyHourMatch[1]!) * 3_600_000,
    );
  }
  ...
  if (dailyMatch) {
    ...
    next.setHours(
      Number.parseInt(dailyMatch[1]!),
      Number.parseInt(dailyMatch[2]!),
      0,
      0,
    );
