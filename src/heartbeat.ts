import type { DB, Task } from "./memory";
...
function calculateNextRun(recurrence: string): Date | null {
	...
	if (everyMinMatch) {
		return new Date(now.getTime() + Number.parseInt(everyMinMatch[1]!) * 60_000);
	}
	...
	if (dailyMatch) {
		...
		next.setHours(Number.parseInt(dailyMatch[1]!), Number.parseInt(dailyMatch[2]!), 0, 0);
