...
function escapeMarkdownV2(text: string): string {
	...
	while ((match = pattern.exec(text)) !== null) {
		...
		if (match.index > cursor) {
			parts.push(text.slice(cursor, match.index).replace(SPECIAL_CHARS, "\\$&"));
		}
...
export class TelegramAdapter {
	...
	constructor(agent: Agent, db: DB) {
		...
		this.chatId = process.env.TELEGRAM_CHAT_ID ? Number(process.env.TELEGRAM_CHAT_ID) : null;
		...
		this.bot.on("message:text", async (ctx) => {
			...
			const exists = await this.db.hasMessageWithMetadata("telegram_message_id", tgMessageId);
			if (exists) {
